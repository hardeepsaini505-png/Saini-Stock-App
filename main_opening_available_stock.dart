...
class FirmProfile {
  ...
  factory FirmProfile.fromJson(Map<String, dynamic> j) => FirmProfile(
...
class AppStore extends ChangeNotifier {
  List<FirmProfile> firms = [];
  int currentFirmIndex = 0;

  FirmProfile get currentFirm => firms[currentFirmIndex];
  String get firmName => currentFirm.name;
  String get address => currentFirm.address;
  String get mobile => currentFirm.mobile;
  List<StockItem> get items => currentFirm.items;

  Future<void> load() async {
    final p = await SharedPreferences.getInstance();
    final rawFirms = p.getString('firms');
    if (rawFirms != null) {
      try {
        firms = (jsonDecode(rawFirms) as List)
            .map((e) => FirmProfile.fromJson(Map<String, dynamic>.from(e)))
            .toList();
      } catch (_) {
        firms = [];
      }
      currentFirmIndex = p.getInt('currentFirmIndex') ?? 0;
    }

    // Migrate the old single-firm data automatically.
    if (firms.isEmpty) {
      final oldName = p.getString('firmName') ?? '';
      final oldAddress = p.getString('address') ?? '';
      final oldMobile = p.getString('mobile') ?? '';
      final rawItems = p.getString('items');
      List<StockItem> oldItems = [];
      if (rawItems != null) {
        try {
          oldItems = (jsonDecode(rawItems) as List)
              .map((e) => StockItem.fromJson(Map<String, dynamic>.from(e)))
              .toList();
        } catch (_) {}
      }
    ...
    if (currentFirmIndex < 0 || currentFirmIndex >= firms.length) {
      currentFirmIndex = 0;
    }
    _sortItems();
  }
  ...
  void _sortItems() {
    for (final f in firms) {
      f.items.sort((a, b) => a.name.toLowerCase().compareTo(b.name.toLowerCase()));
    }
  }
  ...
  Future<void> save() async {
    _sortItems();
    final p = await SharedPreferences.getInstance();
    await p.setString('firms', jsonEncode(firms.map((e) => e.toJson()).toList()));
    await p.setInt('currentFirmIndex', currentFirmIndex);
    // Keep old keys too, so older builds can still open the latest selected firm.
  ...
  Future<void> addFirm(String name, String address, String mobile) async {
    final n = name.trim();
    if (n.isEmpty) return;
    firms.add(FirmProfile(
      id: DateTime.now().microsecondsSinceEpoch.toString(),
      name: n,
      address: address.trim(),
      mobile: mobile.trim(),
    ));
    currentFirmIndex = firms.length - 1;
    await save();
  }

  Future<void> updateFirm(FirmProfile firm, String name, String address, String mobile) async {
    firm.name = name.trim();
    firm.address = address.trim();
    firm.mobile = mobile.trim();
    await save();
  }

  Future<void> switchFirm(int index) async {
    if (index < 0 || index >= firms.length) return;
    currentFirmIndex = index;
    await save();
  }

  Future<bool> deleteFirm(FirmProfile firm) async {
    if (firms.length <= 1) return false;
    final removedIndex = firms.indexOf(firm);
    firms.remove(firm);
    if (currentFirmIndex > removedIndex) currentFirmIndex--;
    if (currentFirmIndex >= firms.length) currentFirmIndex = firms.length - 1;
    await save();
    return true;
  }

  Future<void> setFirm(String n, String a, String m) async {
    await updateFirm(currentFirm, n, a, m);
  }

  Future<void> addItem(String name, String unit, {int openingStock = 0}) async {
    if (name.trim().isEmpty) return;
    if (openingStock < 0) openingStock = 0;
    if (items.any((x) => x.name.toLowerCase() == name.trim().toLowerCase())) return;
    items.add(StockItem(
      id: DateTime.now().microsecondsSinceEpoch.toString(),
      name: name.trim(),
      ...
      stock: openingStock,
      openingStock: openingStock,
      history: openingStock > 0
          ? [StockEntry(type: 'OPENING', qty: openingStock, date: DateTime.now())]
          : [],
    ));
    await save();
  }

  Future<void> setOpeningStock(StockItem item, int newOpeningStock) async {
    if (newOpeningStock < 0) newOpeningStock = 0;
    final delta = newOpeningStock - item.openingStock;
    if (delta != 0) {
      item.stock += delta;
      item.history.add(StockEntry(
        type: 'OPENING',
        qty: delta.abs(),
        date: DateTime.now(),
      ));
    }
    item.openingStock = newOpeningStock;
  ...
  List<StockItem> get availableStock =>
      (items.where((e) => e.stock > 2).toList()
        ..sort((a, b) => a.name.toLowerCase().compareTo(b.name.toLowerCase())));

  List<StockItem> get nilStock =>
      (items.where((e) => e.stock == 0).toList()
        ..sort((a, b) => a.name.toLowerCase().compareTo(b.name.toLowerCase())));
  ...
  Map<String, dynamic> exportBackup() => {
        ...
        'currentFirmIndex': currentFirmIndex,
        'firms': firms.map((e) => e.toJson()).toList(),
        // Keep current-firm fields for backward compatibility with old backups.
        'firmName': firmName,
        'address': address,
        'mobile': mobile,
        'items': items.map((e) => e.toJson()).toList(),
  ...
  Future<void> restoreBackup(Map<String, dynamic> data) async {
    final rawFirms = data['firms'];
    if (rawFirms is List) {
      firms = rawFirms
          .map((e) => FirmProfile.fromJson(Map<String, dynamic>.from(e as Map)))
          .toList();
      if (firms.isEmpty) throw const FormatException('Backup has no firms');
      currentFirmIndex = (data['currentFirmIndex'] as num?)?.toInt() ?? 0;
      if (currentFirmIndex < 0 || currentFirmIndex >= firms.length) currentFirmIndex = 0;
      await save();
      return;
    }

    // Restore an older single-firm backup.
    final rawItems = data['items'];
    if (rawItems is! List) throw const FormatException('Invalid backup file');
    firms = [FirmProfile(
      id: DateTime.now().microsecondsSinceEpoch.toString(),
      name: (data['firmName'] ?? '').toString(),
...
class _HomePageState extends State<HomePage> {
        ...
        actions: [
          PopupMenuButton<int>(
            ...
            onSelected: (index) async {
              if (index == -1) {
                if (context.mounted) Navigator.push(context, MaterialPageRoute(builder: (_) => FirmManagerPage(store: store)));
              } else {
                await store.switchFirm(index);
              }
            },
            itemBuilder: (_) => [
              ...List.generate(store.firms.length, (i) => PopupMenuItem<int>(
                value: i,
                child: Row(children: [
                  Icon(i == store.currentFirmIndex ? Icons.radio_button_checked : Icons.radio_button_off),
                  const SizedBox(width: 8),
                  Expanded(child: Text(store.firms[i].name.isEmpty ? 'Firm ${i + 1}' : store.firms[i].name)),
                ]),
              )),
              const PopupMenuDivider(),
              const PopupMenuItem<int>(value: -1, child: Text('Manage Firms')),
            ],
          ),
          IconButton(
  ...
  void _availableStockPage(BuildContext context) {
    Navigator.push(context, MaterialPageRoute(
      builder: (_) => AvailableStockPage(store: store),
    ));
  }
...
class FirmManagerPage extends StatefulWidget {
  final AppStore store;
  const FirmManagerPage({super.key, required this.store});

  @override
  State<FirmManagerPage> createState() => _FirmManagerPageState();
}

class _FirmManagerPageState extends State<FirmManagerPage> {
  Future<void> _editFirm(FirmProfile? firm) async {
    final n = TextEditingController(text: firm?.name ?? '');
    final a = TextEditingController(text: firm?.address ?? '');
    final m = TextEditingController(text: firm?.mobile ?? '');
    await showDialog(
      context: context,
      builder: (dc) => AlertDialog(
        title: Text(firm == null ? 'Add Firm' : 'Update Firm'),
        content: SingleChildScrollView(child: Column(children: [
          TextField(controller: n, autofocus: true, decoration: const InputDecoration(labelText: 'Firm Name')),
          TextField(controller: a, decoration: const InputDecoration(labelText: 'Address')),
          TextField(controller: m, keyboardType: TextInputType.phone, decoration: const InputDecoration(labelText: 'Mobile Number')),
        ])),
        actions: [
          ...
          FilledButton(onPressed: () async {
            ...
            if (firm == null) {
              await widget.store.addFirm(n.text, a.text, m.text);
            } else {
              await widget.store.updateFirm(firm, n.text, a.text, m.text);
            }
            if (dc.mounted) Navigator.pop(dc);
            setState(() {});
          }, child: Text(firm == null ? 'Add' : 'Update')),
        ],
      ),
    );
  }

  Future<void> _deleteFirm(FirmProfile firm) async {
    if (widget.store.firms.length <= 1) {
      ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('कम से कम एक firm रखना जरूरी है।')));
      return;
    }
    final ok = await showDialog<bool>(
      ...
      builder: (dc) => AlertDialog(
        ...
        content: Text('क्या “${firm.name.isEmpty ? 'Firm' : firm.name}” को delete करना है?\n\nइस firm के सभी items, stock और history भी delete हो जाएंगे.'),
        actions: [
    ...
    if (ok == true) {
      await widget.store.deleteFirm(firm);
      setState(() {});
    }
  }
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(title: const Text('Manage Firms')),
      body: ListView.builder(
        padding: const EdgeInsets.all(12),
        itemCount: widget.store.firms.length,
        itemBuilder: (_, i) {
          final f = widget.store.firms[i];
          final active = i == widget.store.currentFirmIndex;
          return Card(
            child: ListTile(
              leading: CircleAvatar(child: Icon(active ? Icons.check : Icons.business)),
              title: Text(f.name.isEmpty ? 'Firm ${i + 1}' : f.name, style: const TextStyle(fontWeight: FontWeight.bold)),
              subtitle: Text('${f.address}${f.mobile.isEmpty ? '' : '\n${f.mobile}'}\nItems: ${f.items.length}'),
              isThreeLine: true,
              onTap: () async { await widget.store.switchFirm(i); setState(() {}); },
              trailing: Wrap(children: [
                IconButton(icon: const Icon(Icons.edit), onPressed: () => _editFirm(f)),
                IconButton(icon: const Icon(Icons.delete, color: Colors.red), onPressed: () => _deleteFirm(f)),
              ]),
            ),
          );
        },
...
class _StockEntryPageState extends State<StockEntryPage> {
  ...
  Widget build(BuildContext context) {
    ...
    final list = widget.store.items.where((x) => x.name.toLowerCase().contains(q)).toList();
    return Scaffold(
      appBar: AppBar(title: Text(widget.isIn ? 'Stock In' : 'Stock Out')),
      body: Column(children: [
        Padding(padding: const EdgeInsets.fromLTRB(12, 12, 12, 4), child: TextField(
          controller: search,
          onChanged: (_) => setState(() {}),
          decoration: InputDecoration(labelText: 'Search Item', hintText: 'Item name search...', prefixIcon: const Icon(Icons.search), suffixIcon: search.text.isEmpty ? null : IconButton(icon: const Icon(Icons.clear), onPressed: () { search.clear(); setState(() {}); })),
        )),
        Expanded(child: list.isEmpty ? const Center(child: Text('No item found')) : ListView.builder(
          padding: const EdgeInsets.all(12), itemCount: list.length,
          itemBuilder: (_, i) { final item = list[i]; return Card(child: ListTile(title: Text(item.name), subtitle: Text('Balance: ${item.stock} ${item.unit}'), trailing: FilledButton(onPressed: () => _qtyDialog(context, item), child: Text(widget.isIn ? 'IN' : 'OUT')))); },
        )),
      ]),
    );
  }
  ...
  Future<void> _qtyDialog(BuildContext context, StockItem item) async {
    ...
    await showDialog(context: context, builder: (_) => AlertDialog(
      title: Text('${widget.isIn ? 'Stock In' : 'Stock Out'} — ${item.name}'),
      content: TextField(controller: c, keyboardType: TextInputType.number, decoration: InputDecoration(labelText: 'Quantity (${item.unit})')),
      actions: [TextButton(onPressed: () => Navigator.pop(context), child: const Text('Cancel')), FilledButton(onPressed: () async {
        ...
        if (widget.isIn) { await widget.store.stockIn(item, q); } else {
          final ok = await widget.store.stockOut(item, q);
          if (!ok && context.mounted) { ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('Stock Out quantity is greater than available stock.'))); return; }
        }
        if (context.mounted) Navigator.pop(context);
      }, child: const Text('Save'))],
    ));
...
class _BalancePageState extends State<BalancePage> {
  ...
  Color statusColor(int s) { if (s == 0) return Colors.red; if (s <= 2) return Colors.amber.shade800; return Colors.green; }
  String status(int s) => s == 0 ? 'NILL STOCK' : s <= 2 ? 'LOW STOCK' : 'NORMAL';
  @override Widget build(BuildContext context) {
    final q=search.text.trim().toLowerCase();
    final list=widget.store.items.where((x)=>x.name.toLowerCase().contains(q)).toList();
    return Scaffold(appBar: AppBar(title: const Text('Balance Stock')), body: Column(children:[
      Padding(padding:const EdgeInsets.fromLTRB(12,12,12,4),child:TextField(controller:search,onChanged:(_)=>setState((){}),decoration:InputDecoration(labelText:'Search Item',prefixIcon:const Icon(Icons.search),suffixIcon:search.text.isEmpty?null:IconButton(icon:const Icon(Icons.clear),onPressed:(){search.clear();setState((){});})))),
      Expanded(
...
class _StatusPageState extends State<StatusPage> {
  ...
  @override void dispose(){search.dispose();super.dispose();}
  @override Widget build(BuildContext context){
    final all=widget.nil?widget.store.nilStock:widget.store.lowStock;
    final q=search.text.trim().toLowerCase();
    final list=all.where((x)=>x.name.toLowerCase().contains(q)).toList();
    return Scaffold(appBar:AppBar(title:Text(widget.nil?'Nill Stock':'Low Stock'),actions:[IconButton(icon:const Icon(Icons.picture_as_pdf),onPressed:()=>PdfReports.statusPdf(widget.store,widget.nil?'NILL STOCK':'LOW STOCK',all))]),body:Column(children:[
      Padding(padding:const EdgeInsets.fromLTRB(12,12,12,4),child:TextField(controller:search,onChanged:(_)=>setState((){}),decoration:InputDecoration(labelText:'Search Item',prefixIcon:const Icon(Icons.search),suffixIcon:search.text.isEmpty?null:IconButton(icon:const Icon(Icons.clear),onPressed:(){search.clear();setState((){});})))),
      Expanded(child:list.isEmpty?Center(child:Text(widget.nil?'No nill stock items':'No low stock items')):ListView.builder(padding:const EdgeInsets.all(12),itemCount:list.length,itemBuilder:(_,i){final x=list[i];return Card(color:widget.nil?Colors.red.shade50:Colors.amber.shade50,child:ListTile(title:Text(x.name),trailing:Text('${x.stock} ${x.unit}',style:TextStyle(fontWeight:FontWeight.bold,color:widget.nil?Colors.red:Colors.amber.shade900))));}))
    ]));
  }
}
...
class _OrderPageState extends State<OrderPage> {
  ...
  Widget build(BuildContext context) {
    ...
    final allUnique = <String, StockItem>{for (final x in list) x.id: x}.values.toList();
    final q = search.text.trim().toLowerCase();
    final unique = allUnique.where((x) => x.name.toLowerCase().contains(q)).toList();
    ...
    return Scaffold(
      ...
      body: Column(children: [
        Padding(padding: const EdgeInsets.fromLTRB(12, 12, 12, 4), child: TextField(
          controller: search,
          onChanged: (_) => setState(() {}),
          decoration: InputDecoration(labelText: 'Search Item', prefixIcon: const Icon(Icons.search), suffixIcon: search.text.isEmpty ? null : IconButton(icon: const Icon(Icons.clear), onPressed: () { search.clear(); setState(() {}); })),
        )),
        Expanded(child: unique.isEmpty
...
class _ItemsPageState extends State<ItemsPage> {
  ...
  Widget build(BuildContext context) {
    ...
    final list = widget.store.items.where((x) => x.name.toLowerCase().contains(q)).toList();
    return Scaffold(
      appBar: AppBar(title: const Text('Item Master'), actions: [IconButton(icon: const Icon(Icons.add), onPressed: _addItem)]),
      body: Column(children: [
        Padding(padding: const EdgeInsets.fromLTRB(12, 12, 12, 4), child: TextField(
          controller: search, onChanged: (_) => setState(() {}),
          decoration: InputDecoration(labelText: 'Search Item', hintText: 'Item name search...', prefixIcon: const Icon(Icons.search), suffixIcon: search.text.isEmpty ? null : IconButton(icon: const Icon(Icons.clear), onPressed: () { search.clear(); setState(() {}); })),
        )),
        Expanded(child: list.isEmpty ? const Center(child: Text('कोई item नहीं मिला।')) : ListView.builder(
          padding: const EdgeInsets.all(12), itemCount: list.length,
          itemBuilder: (_, i) { final x = list[i]; final c = x.stock == 0 ? Colors.red : x.stock <= 2 ? Colors.amber.shade800 : Colors.green; return Card(child: ListTile(leading: CircleAvatar(child: Icon(Icons.inventory_2, color: c)), title: Text(x.name, style: const TextStyle(fontWeight: FontWeight.bold)), subtitle: Text('Balance: ${x.stock} ${x.unit}'), trailing: Row(mainAxisSize: MainAxisSize.min, children: [IconButton(tooltip: 'Update Item', icon: const Icon(Icons.edit), onPressed: () => _editItem(x)), IconButton(tooltip: 'Delete Item', icon: const Icon(Icons.delete, color: Colors.red), onPressed: () => _deleteItem(x))]))); },
        )),
      ]),
      floatingActionButton: FloatingActionButton.extended(onPressed: _addItem, icon: const Icon(Icons.add), label: const Text('Add Item')),
    );
  }
  ...
  Future<void> _addItem() async {
    ...
    await showDialog(context: context, builder: (dc)=>AlertDialog(
      ...
      content: Column(mainAxisSize: MainAxisSize.min, children: [
        ...
        TextField(controller:opening, keyboardType:TextInputType.number,
      ...
      actions:[
        ...
        FilledButton(onPressed:() async {
          ...
          final q = int.tryParse(opening.text.trim()) ?? 0;
          if (q < 0) return;
          await widget.store.addItem(name,u.text, openingStock:q);
          if(dc.mounted)Navigator.pop(dc); setState((){});
        }, child:const Text('Add')),
      ],
    ));
  }
  ...
  Future<void> _editItem(StockItem item) async {
    ...
    final opening=TextEditingController(text:item.openingStock.toString());
    await showDialog(context:context,builder:(dc)=>AlertDialog(
      ...
      actions:[
        ...
        FilledButton(onPressed:()async{
          ...
          final q=int.tryParse(opening.text.trim()) ?? 0;
          if(q<0)return;
          item.name=name; item.unit=u.text.trim().isEmpty?'pcs':u.text.trim();
          await widget.store.setOpeningStock(item,q);
          if(dc.mounted)Navigator.pop(dc); setState((){});
